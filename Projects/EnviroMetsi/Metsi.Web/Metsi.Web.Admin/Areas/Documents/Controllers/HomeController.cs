...
namespace Metsi.Web.Admin.Areas.Documents.Controllers
{
    [Area("Documents")]
    [Route("Documents/[controller]/[action]")]
    ...
    public class HomeController : Controller
    {
        ...
        [HttpPost]
        public IActionResult ShareFileFolder([FromBody] SelectedFile details, [FromServices] UserManager<ApplicationUser> context)
        {
        ...
        [HttpPost]
        public IActionResult ShareFileFolderByAddress([FromBody] SelectedFile details, [FromServices] DbContext context)
        {
            var model = new ShareFileFolderModel();
            foreach (var item in details.data)
            {
        ...
        [HttpPost("{id}")]
        public IActionResult DoShareFileFolder([FromBody] SharedFolder[] details, string id, [FromServices] DbContext context)
        {
            var employee = context.Set<Employee>().FirstOrDefault(c => c.Id == id);
        ...
        [HttpPost("{mail}")]
        public async Task<IActionResult> DoShareFileFolderByAddress([FromBody] SharedFolder[] details, string mail, [FromServices] DbContext context, [FromServices] IEmailSender emailSender)
        {
            foreach (var item in details)
            {
           ...
           //     await emailSender.SendEmailAsync(new SendEmailDetails("Enviro Metsi Web Admin", "admin@metsi.co.za", "Private User", mail,
           //         $"Folder {item.FolderName} has been shared with you", $"You can access the account by <a href='{HtmlEncoder.Default.Encode(callbackUrl)}'>clicking here</a>."));
            }
            return RedirectToAction("Index", new { path = details[0].RootPath });
        }
        ...
        [AllowAnonymous]
        public async Task<IActionResult> Geust(string id, [FromServices] DbContext context)
        {
            var sharedFolder = await context.Set<SharedFolder>().FirstOrDefaultAsync(c => c.Id == id);
