...
{
    /// <summary>
    /// The training course main controller
    /// </summary>
    [Area("Courses")]
    [Route("Courses/[controller]/[action]")]
    public class HomeController : CourseBaseController
    {
        public PhysicalFileProvider operation;
        public string basePath;
        string root = "wwwroot\\Courses";

        /// <summary>
        /// Default Constructor
        /// </summary>
        /// <param name="service">The injected <see cref="CourseContext"/></param>
        public HomeController(CourseContext service, IHostingEnvironment hostingEnvironment, IFileFactory fileFactory) : base(service, fileFactory)
        {
        ...
        public async Task<IActionResult> ReCreateDocumentsFolder(string id)
        {
            ...
            FileFactory.CreateDirectory(Path.Combine("wwwroot\\Courses", course.FullPath));
            course.DoNotCreateDocumentsFolder = false;
            course.FolderPath = "Courses";

            await Service.UpdateCourseAsync(course);

            return RedirectToAction(nameof(Edit), "Home", new { id = course.Id });
        }
    }
}
