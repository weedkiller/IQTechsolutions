...
namespace Metsi.Web.Admin.Areas.Courses.Controllers
{
    ...
    public class SectionsController : SectionBaseController
    {
        public PhysicalFileProvider operation;
        public string basePath;
        string root = "wwwroot\\Courses\\Modules\\Sections";

        /// <summary>
        /// The default constructor
        /// </summary>
        /// <param name="service">The injected <see cref="ITrainingSectionService"/></param>
        public SectionsController(SectionContext service, IHostingEnvironment hostingEnvironment, IFileFactory fileFactory) : base(service, fileFactory)
        {
        ...
        {
            var section = await Service.GetAsync(id);

            FileFactory.CreateDirectory(Path.Combine("wwwroot\\Courses\\Modules\\Sections", section.FullPath));
            section.FolderPath = "Courses\\Modules\\Sections";
            section.DoNotCreateDocumentsFolder = false;

            await Service.UpdateSectionAsync(section);

            return RedirectToAction(nameof(Edit), "Sections", new { id = section.Id });
        }
    }
}
