...
namespace Iqt.Core.Controllers
{
    ...
    public class CategoryBaseController<T> : Controller where T : EntityBase, new()
    {
        ...
        public async Task<IActionResult> List()
        {
            ...
            var model = (Service.GetAll()).OrderByDescending(c => c.Created);
        ...
        public virtual async Task<IActionResult> Create(CategoryAddEditModel<T> model, string main, string finnish, string sub)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (sub != null)
                {
                    return RedirectToAction(nameof(Create), "Categories", new { parentId = model.Entity.ParentCategoryId, parentCategoryId = model.Entity.Id });
                }
            }
