...
namespace Iqt.Core.Controllers
{
    ...
    {
        protected readonly BlogContextService Service;
        ...
        public BlogBaseController(BlogContextService service, UserManager<ApplicationUser> userManager, SignInManager<ApplicationUser> signInManager, CategoryContext<BlogPost> categoryService)
        {
            Service = service;
        ...
        public IActionResult Index(int? month, string category, string tagString, int? size, int? page)
        {
            ...
            var blogEntryList = Service.GetAll().Where(c => c.Active).OrderBy(c => c.Modified).ThenBy(c => c.Created).ToList();
