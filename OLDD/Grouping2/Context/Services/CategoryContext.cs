...
namespace Grouping.Context.Services
{
    ...
    public class CategoryContext<T> where T : IEntityBase, new()
    {
        ...
        public CategoryContext(IDbContext context, IFileFactory fileFactory)
        {
        ...
        /// </summary>
        /// <param name="context">The injected database context</param>
        public CategoryContext(IDbContext context)
        {
            _context = (DbContext)context;
            _fileFactory = new FileFactory(context);
        }
        ...
        {
            return _context.Set<Category<T>>().Include(c => c.Images).Include(c => c.SubCategories);
        }
        ...
        /// <param name="departmentId">The identity of the parent <see cref="Department{T}"/> of the children that must be retrieved</param>
        /// <returns>An <see cref="IQueryable{T}"/> of parent <see cref="Category{T}"/></returns>
        public IQueryable<Category<T>> GetAllParentCategories(string parentId = null)
        {
        ...
        /// </summary>
        /// <param name="parentId">The identity of the parent <see cref="Department{T}"/> of the children that must be retrieved</param>
        /// <returns>An <see cref="IQueryable{T}"/> of child <see cref="Category{T}"/></returns>
        public IQueryable<Category<T>> GetAllChildCategories(string parentId)
        {
        ...
        {
            var result = GetAllChildCategories(categoryId);
            return result.Count();
        }
