...
namespace Grouping.Core.Context.Services
{
    public class DepartmentContext<T> where T : IEntityBase, new()
    {
        ...
        public DepartmentContext(DbContext context, IFileFactory fileFactory, IApplicationConfiguration configuration)
        {
        ...
        public async Task<Department<T>> AddAsync(DepartmentAddEditModel<T> model)
        {
