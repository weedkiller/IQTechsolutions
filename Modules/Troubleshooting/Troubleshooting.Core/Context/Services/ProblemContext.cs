...
namespace Troubleshooting.Core.Context.Services
{
    ...
    public class ProblemsContext
    {
        ...
        public IQueryable<Cause> GetProblemCausesAsync(string problemId)
        {
            var resultList = _context.Set<Cause>().Include(c => c.CorrectiveActions)
        ...
        {
            _context.Set<CorrectiveAction>().Update(correctiveAction);
            await _context.SaveChangesAsync();
            return correctiveAction;
        }
